...
public class DeleteAppSessions : AbstractFeaturedController
{
    ...
    public async Task<IActionResult> DeleteAllSessionsForUser([FromBody] DeleteAccountRequest request)
    {
        var username = request?.Username;
        if (string.IsNullOrEmpty(username)) return BadRequest("username is required.");
        bool success = await ServerState.UserStore.DeleteAppSessions(username, request?.MobileID);
